...
export async function setupSwapTest(
  ...
  // Seed the oracle cache so swap tests can run. Bid/ask are picked to imply
  // a 1:1 token_0 → token_1 mid (mantissa 10_000 with exp 0) plus 40 bps total
  // spread (20 each side). Real keeper bot pushes derived values from
  // `client::oracle_math`.
  await updatePoolOracle(
    program,
    owner,
    poolAddress,
    new BN(9_980),
...
export async function updatePoolOracle(
  program: Program<NomadAmm>,
  oracleKeeper: Signer,
  poolAddress: PublicKey,
  effectiveBidMantissa: BN,
  effectiveAskMantissa: BN,
  ...
  return await program.methods
    .updatePoolOracle(
      effectiveBidMantissa,
      effectiveAskMantissa,
      priceExponent,
      dynamicFeeRate
    )
    .accounts({
      oracleKeeper: oracleKeeper.publicKey,
      poolState: poolAddress,
    })
    .signers([oracleKeeper])
    .rpc(confirmOptions);
}
...
const DEFAULT_ORACLE_CONFIG = (keeper: PublicKey): OracleConfig => ({
  pythPriceFeedId: 1,
  minSpreadBps: 40,
  inventorySkewEnabled: false,
  inventorySkewDeadzoneBps: 0,
  inventorySkewBpsPerPct: 0,
  inventorySkewMaxBps: 0,
  oracleKeeper: keeper,
});
...
export async function initialize(
  ...
  const oracle = oracleConfig ?? DEFAULT_ORACLE_CONFIG(creator.publicKey);
  ...
  await program.methods
    .initialize(
      ...
      new BN(0),
      oracle.pythPriceFeedId,
      oracle.minSpreadBps,
      oracle.inventorySkewEnabled,
      oracle.inventorySkewDeadzoneBps,
      oracle.inventorySkewBpsPerPct,
      oracle.inventorySkewMaxBps,
      oracle.oracleKeeper
    )
    .accounts({
