...
pub fn initialize(
    ...
    require_neq!(pyth_price_feed_id, 0u32, ErrorCode::InvalidPythFeedId);
    require!(
        (1..=1_000).contains(&min_spread_bps),
        ErrorCode::InvalidMinSpreadBps
    );
    // Inventory skew bounds:
    //   deadzone   — up to 50% (5_000 bps) of value imbalance tolerated silently
    //   bps_per_pct — up to 100 bps of skew per 1% imbalance (very aggressive)
    //   max_bps    — up to 10% (1_000 bps) hard cap on the shift
    require!(
        inventory_skew_deadzone_bps <= 5_000
            && inventory_skew_bps_per_pct <= 100
            && inventory_skew_max_bps <= 1_000,
        ErrorCode::InvalidInventorySkewParam
    );
