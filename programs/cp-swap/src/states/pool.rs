...
pub struct PoolState {
    ...
    pub last_oracle_update_unix: i64,
    /// padding for future updates
    pub padding: [u64; 17],
}

impl PoolState {
    // 8 (disc) + 10*32 (pubkeys) + 5*1 (small u8s) + 7*8 (u64 fields) + 2*1
    // (creator_fee_on + enable_creator_fee) + 6 (padding1) + 2*8 (creator_fees)
    // + 4 (pyth_price_feed_id) + 2 (min_spread_bps) + 1 (inventory_skew_enabled)
    // + 1 (_reserved2) + 2 (deadzone) + 2 (bps_per_pct) + 2 (max_bps) + 2 (_reserved3)
    // + 32 (oracle_keeper) + 2*8 (effective_bid/ask) + 2 (price_exponent)
    // + 6 (_oracle_pad) + 3*8 (dynamic_fee_rate + last_oracle_update_slot/unix)
    // + 17*8 (padding)
    ...
    pub fn initialize(
        ...
        self.pyth_price_feed_id = pyth_price_feed_id;
        self.min_spread_bps = min_spread_bps;
        self.inventory_skew_enabled = if inventory_skew_enabled { 1 } else { 0 };
        self._reserved2 = 0;
        self.inventory_skew_deadzone_bps = inventory_skew_deadzone_bps;
        self.inventory_skew_bps_per_pct = inventory_skew_bps_per_pct;
        self.inventory_skew_max_bps = inventory_skew_max_bps;
        self._reserved3 = 0;
        self.oracle_keeper = oracle_keeper;
        self.effective_bid_mantissa = 0;
        self.effective_ask_mantissa = 0;
        self.price_exponent = 0;
        self._oracle_pad = [0u8; 6];
        self.dynamic_fee_rate = 0;
        self.last_oracle_update_slot = 0;
        self.last_oracle_update_unix = 0;
    ...
    /// Return the effective trade fee rate the swap path should use, falling
    /// back to the AMM-config base rate when the keeper bot has not yet pushed
    /// a dynamic value (e.g. immediately after pool init).
    pub fn effective_trade_fee_rate(&self, base_trade_fee_rate: u64) -> u64 {
        if self.dynamic_fee_rate == 0 {
            base_trade_fee_rate
        } else {
            self.dynamic_fee_rate
        }
    }

    /// Read the cached effective bid/ask mantissas. Returns `None` until the
    /// keeper bot's first `update_pool_oracle` call (signalled by `bid == 0`).
    pub fn cached_oracle_quote(&self) -> Option<(i64, i64, i16)> {
        let bid = self.effective_bid_mantissa;
        let ask = self.effective_ask_mantissa;
        if bid <= 0 || ask <= 0 {
            return None;
        }
        Some((bid, ask, self.price_exponent))
    }
