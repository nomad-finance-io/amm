...
/// Local quote-preview helper for the CLI: read the cached oracle bid/ask
/// from PoolState, pick the side per `trade_direction`, and convert to the
/// raw token-1/token-0 fraction the curve consumes. Mirrors the logic in
/// `swap_base_input.rs`/`swap_base_output.rs` minus the inventory skew —
/// previews only need rough quotes.
fn preview_spot_rate(
    pool_state: &nomad_amm::states::PoolState,
    trade_direction: nomad_amm::curve::TradeDirection,
) -> Option<(u128, u128)> {
    let (bid, ask, exponent) = pool_state.cached_oracle_quote()?;
    let mantissa = match trade_direction {
        nomad_amm::curve::TradeDirection::ZeroForOne => bid,
        nomad_amm::curve::TradeDirection::OneForZero => ask,
    };
    let (num, den) = nomad_amm::curve::oracle_curve::pyth_price_to_raw_fraction(
        mantissa,
        exponent,
        pool_state.mint_0_decimals,
        pool_state.mint_1_decimals,
    )?;
    Some(match trade_direction {
        nomad_amm::curve::TradeDirection::ZeroForOne => (num, den), // out=token_1
        nomad_amm::curve::TradeDirection::OneForZero => (den, num), // out=token_0
    })
}
    ...
    match opts.command {
        RaydiumCpCommands::InitializePool {
            ...
            let initialize_pool_instr = initialize_pool_instr(
                ...
                nomad_amm::create_pool_fee_reveiver::ID,
                random_pool_id,
                init_amount_0,
                init_amount_1,
                open_time,
                // CLI defaults — production users should pass these via flags.
                // SOL/USD = 1 in Pyth Lazer's feed registry; tune per pool.
                1u32,
                40u16,
                false,
                0u16,
        ...
        RaydiumCpCommands::Deposit {
            ...
            let pool_state: nomad_amm::states::PoolState = program.account(pool_id)?;
            // load account
            ...
            let (total_token_0_amount, total_token_1_amount) = pool_state
                ...
                .unwrap();
            // calculate amount
            let results = nomad_amm::curve::CurveCalculator::lp_tokens_to_trading_tokens(
                u128::from(lp_token_amount),
                u128::from(pool_state.lp_supply),
                u128::from(total_token_0_amount),
                u128::from(total_token_1_amount),
                nomad_amm::curve::RoundDirection::Ceiling,
            )
            .ok_or(nomad_amm::error::ErrorCode::ZeroTradingTokens)
            .unwrap();
            println!(
        ...
        RaydiumCpCommands::SwapBaseIn {
            ...
            let (
                ...
                (
                    nomad_amm::curve::TradeDirection::ZeroForOne,
                ...
                (
                    nomad_amm::curve::TradeDirection::OneForZero,
                    total_token_1_amount,
                    total_token_0_amount,
                    user_input_token,
                    spl_associated_token_account::get_associated_token_address(
            ...
            let (spot_num, spot_den) = preview_spot_rate(&pool_state, trade_direction)
                .expect("pool oracle cache not seeded — keeper bot must run before previewing");
            let result = nomad_amm::curve::CurveCalculator::swap_base_input(
                u128::from(actual_amount_in),
                u128::from(total_input_token_amount),
                u128::from(total_output_token_amount),
                pool_state.effective_trade_fee_rate(amm_config_state.trade_fee_rate),
        ...
        RaydiumCpCommands::SwapBaseOut {
            ...
            let (spot_num, spot_den) = preview_spot_rate(&pool_state, trade_direction)
                .expect("pool oracle cache not seeded — keeper bot must run before previewing");
            let result = nomad_amm::curve::CurveCalculator::swap_base_output(
